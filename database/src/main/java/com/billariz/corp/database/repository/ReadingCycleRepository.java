...
public interface ReadingCycleRepository extends RepositoryReadWriteDelete<ReadingCycle, String>
{
    @Query("SELECT a " +
    "FROM com.billariz.corp.database.model.ReadingCycle a " +
    "WHERE (:id IS NULL OR a.id = :id) " +
    "  AND (:label IS NULL OR " +
    "       lower(a.id) LIKE CONCAT('%', lower(:label), '%') OR " +
    "       lower(a.defaultLabel) LIKE CONCAT('%', lower(:label), '%') OR " +
    "       lower(a.otherLabel) LIKE CONCAT('%', lower(:label), '%') OR " +
    "       lower(a.standardLabel) LIKE CONCAT('%', lower(:label), '%')) " +
    "  AND (:periodicity IS NULL OR a.periodicity = :periodicity)")
Page<ReadingCycle> findReadingCycle(@Param("id") String id, 
